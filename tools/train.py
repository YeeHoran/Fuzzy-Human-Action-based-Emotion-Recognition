...
def main():
    ...
    # Check if we need distributed setup
    config_file = sys.argv[1]
    cfg = Config.fromfile(config_file)

    # Assume single GPU unless 'launcher' argument indicates otherwise
    distributed = False  # Default is single GPU training
    if 'launcher' in sys.argv and sys.argv[sys.argv.index('launcher') + 1] != 'none':
        print("Distributed training requested, but we will skip it for single GPU setup.")
    else:
        print("Running on single GPU without distributed training.")

    # If not using distributed training, skip dist initialization
    if distributed:

        print("Initializing distributed setup...")
        if 'dist_params' in cfg:
            init_dist('pytorch', backend=cfg.dist_params.get('backend', 'gloo'))
        else:
            print("No dist_params found, skipping distributed initialization.")
    else:
        print("Single GPU mode: Skipping distributed setup.")
        rank = 0  # Single GPU means the process is rank 0
        world_size = 1  # Single process
        print("Skipping distributed training setup.")
    #####################################
    ...
    if not hasattr(cfg, 'dist_params'):
        ...
        cfg.dist_params = dict(backend=None)

#    init_dist(args.launcher, **cfg.dist_params)
    #rank, world_size = get_dist_info()
    rank, world_size = 0, 1
    #cfg.gpu_ids = range(world_size)
    cfg.gpu_ids = [0]  # Single GPU setup
    ...
    #train_model(model, datasets, cfg, validate=args.validate, test=test_option, timestamp=timestamp, meta=meta, distributed=False)
    train_model(
        model,
        datasets,
        cfg,
